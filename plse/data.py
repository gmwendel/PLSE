...
class DataGenerator(Sequence):
    def __init__(self, x1, x2, y, batch_size=2 ** 13, shuffle=True, augment_data=True, max_shift_left=10,
                 max_shift_right=10):
        """
        DataGenerator constructor.

        Parameters:
            x1 (np.ndarray): The input data for the model, 2d waveform data.
            x2 (np.ndarray): The input data for the model, pmt_types.
        ...
        """
        self.x1 = x1  # waveforms
        self.x2 = x2  # pmt_types
        self.y = y
    ...
    def __getitem__(self, index):
        ...
        batch_x1 = self.x1[indexes]
        batch_x2 = self.x2[indexes]
        batch_y = self.y[indexes]

        # Apply data augmentation to batch_x1 (waveforms)
        if self.augment_data:
            batch_x1 = self._augment_data(batch_x1)

        # Prepare inputs as a dictionary matching the model's input names
        print("batch_x1 shape before reshape:", batch_x1.shape)
        ...
        print("batch_x1 shape after reshape:", batch_x1.shape)
